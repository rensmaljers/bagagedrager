-- ============================================
-- 062: GC-straf ook voor gekozen renners ZONDER uitslag-rij (DNS / niet in PCS)
-- ============================================
-- Migratie 059 paste de DNF-straf alleen toe bij is_late OF een uitslag-rij
-- met dnf=true. Een renner die niet startte en door PCS helemaal niet in de
-- uitslag wordt vermeld, heeft géén stage_results-rij: de LEFT JOIN gaf NULL,
-- time_seconds werd 0 en het tijdverschil GREATEST(0 - winnertijd, 0) = 0.
-- Zo'n DNS-renner kreeg dus de winnaarstijd (0 achterstand) i.p.v. de straf.
--
-- Fix: "niet gefinisht" = geen finish-positie (sr.finish_position IS NULL).
-- Dat dekt zowel de dnf-rij (positie NULL) als de ontbrekende rij. In
-- general_classification zit dit in de CTE-alias `dnf`, in stage_picks_public
-- per trigger inline. De strafexpressie zelf is ongewijzigd t.o.v. 059.
  ...
  SUM(
    CASE
      WHEN pt.is_late OR pt.dnf THEN COALESCE(
        -- slechtste tijdverschil van gekozen renners die finishten
        (SELECT MAX(sr2.time_seconds) - (SELECT winner_time FROM stage_winner_times WHERE stage_id = pt.stage_id)
        ...
        -- alle gekozen renners DNF: slechtste tijdverschil van het hele veld
...
-- stage_picks_public: zelfde "niet gefinisht"-regel inline per trigger
...
SELECT
  ...
  CASE
    WHEN p.is_late OR COALESCE(sr.dnf, false) OR sr.finish_position IS NULL THEN COALESCE(
      -- slechtste tijdverschil van gekozen renners die finishten
      (SELECT MAX(sr2.time_seconds) - (SELECT winner_time FROM stage_winner_times WHERE stage_id = p.stage_id)
         FROM stage_results sr2
