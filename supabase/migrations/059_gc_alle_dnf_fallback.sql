-- ============================================
-- 059: GC-straf fallback als álle gekozen renners DNF zijn
-- ============================================
-- De strafberekening (DNF/te laat = slechtste tijdverschil van gekozen
-- renners die finishten) gaf 0 als GEEN ENKELE gekozen renner finishte
-- (lege MAX → COALESCE 0). Een speler met een DNF-pick betaalde dan
-- helemaal niets. Nieuw: fallback naar het slechtste tijdverschil van
-- het hele veld op die etappe.
-- Views identiek aan 040 (general_classification) en 039
-- (stage_picks_public), alleen de strafexpressie is vervangen.
...
SELECT
  ...
  CASE
    WHEN p.is_late OR COALESCE(sr.dnf, false) THEN COALESCE(
      -- slechtste tijdverschil van gekozen renners die finishten
