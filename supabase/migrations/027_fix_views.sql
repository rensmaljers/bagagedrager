-- ============================================
-- FIX 027: Drie correcties in general_classification en stage_picks_public
--
-- 1. total_time_no_bonif + total_bonification terug in general_classification
--    (frontend gebruikt beide kolommen voor weergave)
-- 2. DNF-straf = slechtste tijdsverschil van een GEKOZEN renner in die etappe,
--    niet van de laatste finisher overall
-- 3. Punten- en bergklassement: deelpenalty verwijderd (was per ongeluk
--    teruggezet in migration 026)
-- ============================================
...
SELECT
  ...
  pt.user_id,
  pr.display_name,
  -- Tijdsverschillen zonder bonificatie
  SUM(
    CASE
      WHEN pt.is_late OR pt.dnf THEN (
        -- Straf: slechtste tijdsverschil van een gekozen renner die wél gefinisht is
        SELECT COALESCE(
  ...
  -- Totale tijd met bonificatie (= total_time_no_bonif - total_bonification)
  SUM(
    CASE
      WHEN pt.is_late OR pt.dnf THEN (
        SELECT COALESCE(
