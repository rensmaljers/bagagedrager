-- ============================================
-- FIX 051: DNF renners mogen opnieuw gekozen worden
-- Als een renner DNF heeft in een etappe, telt die keuze niet mee
-- als "al gebruikt" voor volgende etappes.
-- ============================================

-- submit_pick: DNF picks uitsluiten van al-gebruikt check
...
BEGIN
  ...
  -- DNF picks tellen niet mee als "gebruikt"
  SELECT EXISTS(
    ...
    WHERE p.user_id = v_user_id
      ...
      AND NOT EXISTS (
        ...
        WHERE sr.stage_id = p.stage_id
          AND sr.rider_id = p.rider_id
          AND sr.dnf = true
      )
  ) INTO v_already_used;
...
-- admin_upsert_pick: DNF picks uitsluiten van al-gebruikt check
...
-- assign_random_riders: DNF picks uitsluiten van al-gebruikt check
    ...
    WHERE r.competition_id = v_comp_id
      AND r.id NOT IN (
        ...
        WHERE pk.user_id = v_user.user_id
          ...
          AND NOT EXISTS (
            SELECT 1 FROM stage_results sr
            WHERE sr.stage_id = pk.stage_id
              AND sr.rider_id = pk.rider_id
              AND sr.dnf = true
          )
      )
    ORDER BY random()
