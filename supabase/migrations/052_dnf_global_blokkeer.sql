-- ============================================
-- FIX 052: DNF renners globaal geblokkeerd voor nieuwe keuzes
-- Corrigeert 051: DNF picks van jezelf tellen weer mee als "al gebruikt".
-- Nieuw: renners die in enige etappe van de competitie DNF zijn, kunnen
-- door niemand meer gekozen worden voor volgende etappes.
-- ============================================

-- submit_pick: herstel origineel "al gebruikt" + voeg globale DNF-check toe
...
BEGIN
  ...
  -- Check 1: renner al gebruikt in andere etappe van dezelfde competitie
  SELECT EXISTS(
  ...
  -- Check 2: renner heeft DNF in enige etappe van deze competitie
  SELECT EXISTS(
...
-- admin_upsert_pick: herstel origineel + globale DNF-check (admin kan overschrijven)
...
-- assign_random_riders: herstel origineel + sluit DNF renners uit
...
BEGIN
  ...
  LOOP
    ...
    WHERE r.competition_id = v_comp_id
      -- Sluit renners uit die al gebruikt zijn
      AND r.id NOT IN (
      ...
      -- Sluit renners uit die DNF hebben in de koers
      AND NOT EXISTS (
        SELECT 1 FROM stage_results sr
        JOIN stages s ON s.id = sr.stage_id
