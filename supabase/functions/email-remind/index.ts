    ...
    for (const p of unpicked) {
      ...
      const res = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          to: email,
          subject: `⏰ Keuze deadline nadert — ${stageName}`,
          html: `
            <p>Hoi ${p.display_name || "wielrenner"},</p>
            <p>Je hebt nog <strong>geen renner gekozen</strong> voor <strong>${stageName}</strong>.</p>
            <p>Deadline: <strong>${deadlineStr}</strong></p>
            <p><a href="${appUrl}/#pick">Maak je keuze</a></p>
            <p style="color:#888; font-size:0.85em;">Je ontvangt deze mail omdat je meedoet aan Bagagedrager.</p>
          `,
        }),
      });
